...
import { SafeHTML, SafeText, SafeLink, sanitizeUtils } from '../components/SafeComponents.jsx'
import { SecurityProvider, SecurityValidator } from '../components/SecurityProvider.jsx'
...
describe('Security Configuration', () => {
  describe('Input Sanitization', () => {
    ...
    it('should validate URLs', () => {
      expect(sanitization.url('https://example.com')).toBe('https://example.com/')
  ...
  describe('Security Validation', () => {
    it('should detect suspicious patterns', () => {
      ...
      expect(validation.checkSuspiciousPatterns('<script>alert(1)</script>')).toBe(false)
      expect(validation.checkSuspiciousPatterns('javascript:alert(1)')).toBe(false)
      expect(validation.checkSuspiciousPatterns('onclick="malicious()"')).toBe(false)
    })

    it('should validate file uploads', () => {
      const validFile = new File([''], 'test.jpg', { type: 'image/jpeg', size: 1024 })
      const invalidTypeFile = new File([''], 'test.exe', { type: 'application/x-executable' })
      const oversizeFile = new File(['x'.repeat(6 * 1024 * 1024)], 'large.jpg', { type: 'image/jpeg' })
...
describe('Safe Components', () => {
  ...
  describe('SafeLink', () => {
    ...
    it('should add security attributes to external links', () => {
      render(<SafeLink href="https://example.com" external>External Link</SafeLink>)
...
describe('Sanitize Utils', () => {
  ...
  describe('htmlContent', () => {
    it('should allow specified HTML tags', () => {
      const html = '<p>Paragraph</p><script>alert(1)</script><strong>Bold</strong>'
  ...
  describe('url', () => {
    it('should validate and return safe URLs', () => {
      expect(sanitizeUtils.url('https://example.com')).toBe('https://example.com/')
