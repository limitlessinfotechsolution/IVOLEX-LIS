...
export default function MyAccountScreen() {
  ...
  const ProfileSection = () => (
    <div className="space-y-6">
      <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
        <h2 className="text-2xl font-bold text-foreground">{t('account.profile', 'Profile')}</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-surface border border-border rounded-xl p-4">
          ...
          <div className="text-sm text-foreground/60">{t('account.totalOrders', 'Total Orders')}</div>
        </div>
        <div className="bg-surface border border-border rounded-xl p-4">
          <div className="text-2xl font-bold text-green-600 mb-1">
            {formatCurrency(orders.reduce((sum, order) => sum + (order.totals?.total || 0), 0))}
          </div>
          <div className="text-sm text-foreground/60">{t('account.totalSpent', 'Total Spent')}</div>
        </div>
        <div className="bg-surface border border-border rounded-xl p-4">
          <div className="text-2xl font-bold text-blue-600 mb-1">0</div>
          <div className="text-sm text-foreground/60">{t('account.rewardPoints', 'Reward Points')}</div>
        </div>
      </div>
    </div>
  )
  ...
  const OrdersSection = () => (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-foreground">{t('account.orderHistory', 'Order History')}</h2>
      ...
      {orders.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-foreground/60 mb-6">
            {t('account.noOrdersDesc', 'Start shopping to see your orders here')}
          </p>
            ...
            return (
              ...
              >
                ...
                <div className={`grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 ${isRTL ? 'text-right' : 'text-left'}`}>
                  <div>
                    <span className="text-sm text-foreground/60">{t('common.total', 'Total')}: </span>
                    <span className="font-semibold text-foreground">
                      {formatCurrency(order.totals?.total || 0)}
                    </span>
                  </div>
                  <div>
                    <span className="text-sm text-foreground/60">{t('account.items', 'Items')}: </span>
                    <span className="font-semibold text-foreground">
                  ...
                  {order.trackingNumber && (
                    <div>
                      <span className="text-sm text-foreground/60">{t('account.tracking', 'Tracking')}: </span>
                ...
                {order.items && order.items.length > 0 && (
                  <div className="flex -space-x-2 mb-4">
                    {order.items.slice(0, 3).map((item, index) => (
                      <div key={index} className="w-10 h-10 rounded-full border-2 border-background overflow-hidden bg-background">
                        <img 
  ...
  const WishlistSection = () => (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-foreground">{t('account.wishlist', 'Wishlist')}</h2>
      <div className="text-center py-12">
  ...
  const SecuritySection = () => (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-foreground">{t('account.security', 'Security & Privacy')}</h2>
        ...
        {twoFactorEnabled && (
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
              ...
              <span className="text-sm text-green-800">Your account is protected with 2FA</span>
            </div>
  ...
  const AddressesSection = () => (
    <div className="space-y-6">
      <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
        <h2 className="text-2xl font-bold text-foreground">{t('account.addresses', 'Addresses')}</h2>
        <button className="btn btn-primary">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {savedAddresses.map((address) => (
          <div key={address.id} className="bg-surface border border-border rounded-xl p-6">
            <div className={`flex items-start justify-between mb-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
              <div className={isRTL ? 'text-right' : 'text-left'}>
            ...
            <div className={`text-foreground/70 ${isRTL ? 'text-right' : 'text-left'}`}>
              <p>{address.street}</p>
  ...
  const NotificationSection = () => (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-foreground">{t('account.notifications', 'Notification Preferences')}</h2>
            ...
            <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
              <div className={isRTL ? 'text-right' : 'text-left'}>
                ...
                <p className="text-sm text-foreground/60">
                  {key === 'orderUpdates' && 'Receive notifications about your orders'}
                  {key === 'promotions' && 'Get notified about special offers and deals'}
                  {key === 'newsletters' && 'Weekly newsletters with new products'}
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    toast.success(`${key} notifications ${e.target.checked ? 'enabled' : 'disabled'}`)
                  }}
  ...
  const PaymentsSection = () => (
    <div className="space-y-6">
      <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
        <h2 className="text-2xl font-bold text-foreground">{t('account.payments', 'Payment Methods')}</h2>
        <button className="btn btn-primary">
      ...
      <div className="space-y-4">
        {savedPayments.map((payment) => (
          <div key={payment.id} className="bg-surface border border-border rounded-xl p-6">
            <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
              <div className={`flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                <div className="p-2 bg-blue-100 rounded-lg">
                  {payment.type === 'upi' && <Smartphone size={20} className="text-blue-600" />}
                  {payment.type === 'card' && <CreditCard size={20} className="text-blue-600" />}
                </div>
                <div className={isRTL ? 'text-right' : 'text-left'}>
                  <h3 className="font-semibold text-foreground">{payment.label}</h3>
  ...
  const LoginHistorySection = () => (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-foreground">{t('account.loginHistory', 'Login History')}</h2>
      
      <div className="space-y-4">
        {loginHistory.map((session) => (
          <div key={session.id} className="bg-surface border border-border rounded-xl p-6">
            <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
              <div className={`flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={isRTL ? 'text-right' : 'text-left'}>
                  ...
                  <p className="text-xs text-foreground/50">{formatDate(session.timestamp)}</p>
                </div>
              </div>
              <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                {session.isCurrent && (
  ...
  const renderActiveSection = () => {
    switch (activeSection) {
      ...
      case 'loginHistory': return <LoginHistorySection />
      default: return (
        <div className="text-center py-12">
          <h3 className="text-lg font-medium text-foreground mb-2">
            ...
            <div className="bg-surface border border-border rounded-xl p-6">
              <div className={`flex items-center gap-3 mb-6 ${isRTL ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={isRTL ? 'text-right' : 'text-left'}>
                  ...
                  <p className="text-sm text-foreground/60">{t('account.welcomeBack', 'Welcome back')}</p>
                </div>
              </div>
              ...
              <nav className="space-y-2">
                ...
                <button className={`w-full flex items-center gap-3 px-3 py-3 text-red-600 hover:bg-red-50 rounded-lg transition-all ${
                  isRTL ? 'flex-row-reverse text-right' : 'text-left'
                }`}>
                  <LogOut size={20} />
                  <span className="font-medium">{t('account.logout', 'Logout')}</span>
                </button>
              </nav>
            </div>
          </div>
