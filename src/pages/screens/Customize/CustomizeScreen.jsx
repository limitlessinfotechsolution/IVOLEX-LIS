...
import { 
  ArrowRight, ArrowLeft, Upload, X, FileText, Image, 
...
const SEGMENT_CATEGORIES = {
  leather: [
    'Custom Bags', 'Bespoke Wallets', 'Personalized Belts', 
    'Corporate Gifts', 'Custom Accessories', 'Repair Services'
  ],
  electronics: [
    'Custom Solutions', 'Bulk Orders', 'Corporate Electronics', 
    'Modified Devices', 'Branded Products', 'Technical Services'
  ],
  furniture: [
    'Custom Furniture', 'Interior Design', 'Office Solutions', 
    'Home Makeover', 'Commercial Spaces', 'Restoration'
  ]
}
...
export default function CustomizationFlow() {
  ...
  const submitForm = async () => {
    ...
    alert('Customization request submitted successfully! We will contact you within 24 hours.')
  }
  ...
  const isStepValid = () => {
    switch (currentStep) {
      ...
      case 1: // Project
        return formData.category && formData.projectTitle && formData.description
      case 2: // Requirements
    ...
    <div className="min-h-screen py-12" style={{ backgroundColor: theme.colors.background }}>
      <div className="max-w-4xl mx-auto px-6">
        ...
        >
          ...
          >
            Custom {activeSegment.charAt(0).toUpperCase() + activeSegment.slice(1)} Request
          </h1>
          <p 
            className="text-xl max-w-2xl mx-auto"
            style={{ color: theme.colors.muted }}
          >
            Tell us about your vision, and our experts will bring it to life with precision and care.
          </p>
        </motion.div>
              ...
              <div key={step.id} className="flex flex-col items-center">
                <motion.div
                  ...
                  style={{
                    ...
                    borderColor: index <= currentStep ? theme.colors.primary : undefined
                  }}
          ...
          <AnimatePresence mode="wait">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {SEGMENT_CATEGORIES[activeSegment].map((category) => (
                      <motion.button
                        ...
                        style={{
                          backgroundColor: formData.category === category ? theme.colors.primary : 'transparent'
                        }}
            ...
            {currentStep === 2 && (
              <motion.div
                ...
                className="space-y-6"
              >
                <h2 className="text-2xl font-bold text-foreground mb-6">Requirements & Specifications</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {['Personalization/Engraving', 'Premium Materials', 'Custom Packaging', 'Express Delivery'].map((feature) => (
                        <label key={feature} className="flex items-center gap-3">
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                updateFormData('customizations', [...formData.customizations, feature])
                              } else {
                                updateFormData('customizations', formData.customizations.filter(f => f !== feature))
                              }
                            }}
            ...
            {currentStep === 3 && (
              <motion.div
                ...
                className="space-y-6"
              >
                <h2 className="text-2xl font-bold text-foreground mb-6">Files & References</h2>
                <p className="text-foreground/60 mb-6">
                  Upload images, sketches, CAD files, or any reference materials that help us understand your vision.
                </p>
                ...
                >
                  ...
                  <p className="text-foreground/60 text-sm">
                    Supports: Images (JPG, PNG), Documents (PDF), CAD files (DWG, DXF)<br />
                ...
                {formData.uploadedFiles.length > 0 && (
                  <div className="space-y-3">
                    <h4 className="font-medium text-foreground">Uploaded Files ({formData.uploadedFiles.length})</h4>
                    {formData.uploadedFiles.map((file) => (
                      <div key={file.id} className="flex items-center gap-4 p-4 bg-background rounded-segment-lg border border-border">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {['low', 'normal', 'high'].map((urgency) => (
                        <button
                          ...
                          style={{
                            backgroundColor: formData.urgency === urgency ? theme.colors.primary : 'transparent'
                          }}
                        >
                          {urgency}
                        </button>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {BUDGET_RANGES.map((range) => (
                      <button
                        ...
                        style={{
                          backgroundColor: formData.budget === range.value ? theme.colors.primary : 'transparent'
                        }}
                      >
            ...
            {currentStep === 5 && (
              <motion.div
                ...
                className="space-y-8"
              >
                <h2 className="text-2xl font-bold text-foreground mb-6">Review Your Request</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="p-6 bg-background rounded-segment-lg border border-border">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <p><span className="text-foreground/60">Email:</span> {formData.email}</p>
                      <p><span className="text-foreground/60">Phone:</span> {formData.phone}</p>
                      {formData.company && <p><span className="text-foreground/60">Company:</span> {formData.company}</p>}
                    </div>
                  </div>
                  ...
                  <div className="p-6 bg-background rounded-segment-lg border border-border">
                    ...
                    <div className="space-y-2 text-sm">
                      <p><span className="text-foreground/60">Category:</span> {formData.category}</p>
                      <p><span className="text-foreground/60">Title:</span> {formData.projectTitle}</p>
                      <p><span className="text-foreground/60">Quantity:</span> {formData.quantity}</p>
                    </div>
                  </div>
                  ...
                  <div className="p-6 bg-background rounded-segment-lg border border-border">
                    ...
                    <div className="space-y-2 text-sm">
                      <p><span className="text-foreground/60">Timeline:</span> {formData.timeline}</p>
                      <p><span className="text-foreground/60">Budget:</span> {BUDGET_RANGES.find(b => b.value === formData.budget)?.label}</p>
                      <p><span className="text-foreground/60">Urgency:</span> {formData.urgency}</p>
                    </div>
                  </div>
                  ...
                  <div className="p-6 bg-background rounded-segment-lg border border-border">
                    ...
                    <div className="space-y-2 text-sm">
                      <p><span className="text-foreground/60">Files:</span> {formData.uploadedFiles.length} uploaded</p>
                      {formData.customizations.length > 0 && (
                        <p><span className="text-foreground/60">Features:</span> {formData.customizations.join(', ')}</p>
                      )}
                    </div>
                  </div>
                </div>
                  ...
                  <label className="flex items-start gap-3 cursor-pointer">
                    ...
                    <div className="text-sm">
                      ...
                      <p className="text-foreground/60 leading-relaxed">
                        I agree to the terms and conditions and consent to IVOLEX processing my personal data 
                        for the purpose of this customization request. I understand that this is a quote request 
                        and not a binding order until confirmed.
                      </p>
                    </div>
                  </label>
                </div>
                ...
                <div className="text-center p-6 bg-accent/5 rounded-segment-lg border border-accent/20">
                  ...
                  <p className="text-foreground/60">
                    ...
                    and timeline for your custom project.
                  </p>
                </div>
              </motion.div>
            )}
